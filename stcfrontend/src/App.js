...
//   const [labSubjects, setLabSubjects] = useState([{ subject: 'CNS Lab', teacher: 'hdc' ,frequency:1},{ subject: 'LP Lab', teacher: 'ydbd',frequency:2 },{ subject: 'DBMS Lab', teacher: 'tdbjc',frequency:2 }]);
...
//                 onChange={(e) => handleLabSubjectChange(index, 'frequency', Number(e.target.value))}
//               />
//             </div>
//           ))}
//           <button
//             type="button"
//             className="btn btn-secondary mb-3"
//             onClick={() => setLabSubjects([...labSubjects, { subject: '', teacher: '', frequency: 0 }])}
//           >
//             Add Lab Subject
//           </button>
//         </div>
...
function App() {
  ...
  const [subjects, setSubjects] = useState([
    { subject: 'CNS', teacher: 'mr. xyz' },
    { subject: 'SPOS', teacher: 'mr. abc' },
    { subject: 'DBMS', teacher: 'mr. hfb' },
    { subject: 'HCI', teacher: 'mr. yrb' },
    { subject: 'TOC', teacher: 'mr. apn' }
  ]);
  const [labSubjects, setLabSubjects] = useState([
    { subject: 'CNS Lab', teacher: 'hdc', frequency: 1 },
    { subject: 'LP Lab', teacher: 'ydbd', frequency: 2 },
    { subject: 'DBMS Lab', teacher: 'tdbjc', frequency: 2 }
  ]);
  const [startTime, setStartTime] = useState(9); // 9 AM
  const [endTime, setEndTime] = useState(17); // 5 PM
  const [lecDuration, setLecDuration] = useState(60);
  const [labDuration, setLabDuration] = useState(120);
  const [days, setDays] = useState(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]);
  const [lunchBreak, setLunchBreak] = useState({ start: 12, end: 13 }); // New state for lunch break
  const [timetable, setTimetable] = useState([]);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false); // Add loading state
  ...
  const handleSubmit = async () => {
    const response = await fetch('http://localhost:5000/generate_timetable', {
      ...
      body: JSON.stringify({
        ...
        lab_duration: Number(labDuration),
        days,
        lunch_break: lunchBreak // Include lunch break in request
      }),
    });
      ...
      <form>
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => setLunchBreak({ ...lunchBreak, start: Number(e.target.value) })}
          />
        </div>
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => setLunchBreak({ ...lunchBreak, end: Number(e.target.value) })}
          />
        </div>
        ...
        <button
          ...
          disabled={loading}
          style={{marginBottom:'20px'}} // Disable button while loading
        >
